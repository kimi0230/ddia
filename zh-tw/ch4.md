...
- [第四章：編碼與演化](#第四章編碼與演化)
  - [編碼資料的格式](#編碼資料的格式)
    - [語言特定的格式](#語言特定的格式)
    - [JSON、XML和二進位制變體](#jsonxml和二進位制變體)
      - [二進位制編碼](#二進位制編碼)
    - [Thrift與Protocol Buffers](#thrift與protocol-buffers)
      - [欄位標籤和模式演變](#欄位標籤和模式演變)
      - [資料型別和模式演變](#資料型別和模式演變)
    - [Avro](#avro)
      - [Writer模式與Reader模式](#writer模式與reader模式)
      - [模式演變規則](#模式演變規則)
      - [但Writer模式到底是什麼？](#但writer模式到底是什麼)
      - [動態生成的模式](#動態生成的模式)
      - [程式碼生成和動態型別的語言](#程式碼生成和動態型別的語言)
    - [模式的優點](#模式的優點)
  - [資料流的型別](#資料流的型別)
    - [資料庫中的資料流](#資料庫中的資料流)
      - [在不同的時間寫入不同的值](#在不同的時間寫入不同的值)
      - [歸檔儲存](#歸檔儲存)
    - [服務中的資料流：REST與RPC](#服務中的資料流rest與rpc)
      - [Web服務](#web服務)
      - [遠端過程呼叫（RPC）的問題](#遠端過程呼叫rpc的問題)
      - [RPC的當前方向](#rpc的當前方向)
      - [資料編碼與RPC的演化](#資料編碼與rpc的演化)
    - [訊息傳遞中的資料流](#訊息傳遞中的資料流)
      - [訊息代理](#訊息代理)
